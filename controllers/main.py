...
class GestpayController(http.Controller):
    ...
    @http.route(['/payment/gestpay/s2s/create_json'], type='json', auth='public')
    def gestpay_s2s_create_json(self, **kwargs):
    ...
    @http.route(['/payment/gestpay/s2s/create'], type='http', auth='public')
    def gestpay_s2s_create(self, **post):
    ...
    @http.route(['/payment/gestpay/s2s/create_json_3ds'], type='json', auth='public', csrf=False)
    def gestpay_s2s_create_json_3ds(self, verify_validity=False, **kwargs):
        if not kwargs.get('partner_id'):
            kwargs = dict(kwargs, partner_id=request.env.user.partner_id.id)
    ...
    @http.route(['/payment/gestpay/create_charge'], type='json', auth='public')
    def gestpay_create_charge(self, **post):
